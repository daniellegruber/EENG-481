...
[ds1, qInterp] = genConfigTrajectoryNoInterferenceAvodiance({'letter_n', 'letter_x'}, jointNames);
%supplyInputToUserInputMdlByDs(mdl, ds1);
intermediatePointProp = [0.3, 0.5];

x_offset = {[0 0 0.02 0 -0.01], [0 0 0.02 0 -0.01]};
y_offset = {[0 0 0 0 0.01], [0 0 0 0 0.02]};
z_offset = {[0 0 0.02 0 -0.01], [0 0 0.02 0 -0.01]};
...
for i = 1:length(intermediatePointProp)
    ...
    save(['Configs', filesep, 'transition_n_to_x_', num2str(i), '.mat'], "jointValues");
    ds = jointValuesToInputSignals(jointValues, jointNames, 0.001, 2, '');
    supplyInputToUserInputMdlByDs(mdl, ds);
end
% Compare trajectories
[ds2, ~] = genConfigTrajectoryNoInterferenceAvodiance({'letter_n', 'transition_n_to_x_1','transition_n_to_x_2', 'letter_x'}, jointNames);
