...
%% Letter r -> something
toLetter = 'z';
%[ds1, qInterp] = genConfigTrajectoryNoInterferenceAvoidance({'letter_r', ['letter_', toLetter]}, jointNames);
[ds1, qInterp] = genConfigTrajectoryNoInterferenceAvoidance({'letter_r', 'letter_z_stage_1'}, jointNames);
...
%% Letter r -> something adjustments
...
save(['Configs', filesep, 'transition_r_to_', toLetter, '.mat'], "jointValues");
...
[ds2, ~] = genConfigTrajectoryNoInterferenceAvoidance({'letter_r', ['transition_r_to_', toLetter], ['letter_', toLetter]}, jointNames);

supplyInputToUserInputMdlByDs(mdl, ds2);
pause(8);

%% Letter n -> something (multiple fingers problematic)
...
x_offset = [0 0 0 0 0];
y_offset = [0 0 0 0 0];
z_offset = [0 0 0.02 0 -0.02];

% to letter t
% x_offset = [0 0 0.02 0.02 0];
% y_offset = [0 0 0 0 0.01];
% z_offset = [0 0 0.02 0.02 -0.01];
