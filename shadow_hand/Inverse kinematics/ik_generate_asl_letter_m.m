...
xoffset_from_palm = [0 0.04 0.04 0.04];
yoffset_from_knuckle = [0.012, 0.005 0, -0.005];
zoffset_from_palm = [0.04, 0.06, 0.06, 0.06];

afterAdjustments = {};

for fingerIdx = [1 5 2 3 4] % do thtip after lftip
...
if fingerIdx == 5 % Thumb
    ...
    % trvec_target(1) = trvec_target(1) + 0.015;
...
disp('thtip offsets:\n')
thtip_offset = trvec(thtip_to_world) - trvec_lfknuckle;
disp(thtip_offset)
...
%% Generate intermediate qWaypoint in which RF, MF, FF lifted 
...
for fingerIdx = [2 3 4 5]
    ...
    if fingerIdx == 5 
        ...
        if toOverOrUnderIdx == 1
            ...
            % finger
            save(['Configs', filesep, 'letter_m_to_over.mat'], "jointValues");
        else
            % For transitions to signs where the thumb is under another
            % finger
            save(['Configs', filesep, 'letter_m_to_under.mat'], "jointValues");
        end
    end
    valuesPrev = solJointValues;
    q0 = jointValuesToConfigObj(solJointValues, jointNames); % Initial config for next iteration
