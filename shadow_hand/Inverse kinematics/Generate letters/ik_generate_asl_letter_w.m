...
xoffset_from_palm = 0;
yoffset_from_knuckle = 0.0145;
zoffset_from_palm = 0.04;

afterAdjustments = {'RFJ4', deg2rad(-10),'FFJ4', deg2rad(-10)};
...
if fingerIdx == 5 % Thumb
    ...
    lfmiddle_to_world = se3(getTransform(rbt,q0,"lfmiddle","world"));

    % Create target translation
    trvec_lfmiddle = trvec(lfmiddle_to_world); 

    trvec_target = trvec_lfmiddle;
    % trvec_target(1) = trvec_target(1) + 0.01;
    trvec_target(2) = trvec_target(2) + 0.005;
...
% supplyInputToUserInputMdlByMat(mdl, 'Signals/signals_after_solving_RF.mat');
...
disp('thtip offsets:\n')
thtip_offset = trvec(thtip_to_world) - trvec_lfmiddle;
disp(thtip_offset)
