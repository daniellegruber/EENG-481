function [ds, qInterp] = genConfigTrajectoryFromInputV3(signSeq, jointNames)
...
for i = 1:length(signSeq)
    ...
    if i < length(signSeq)
        ...
        if strcmp(signSeq{i}, 'letter_z') && ~strcmp(signSeq{i+1}, 'letter_x') && ~strcmp(signSeq{i+1}, 'letter_o')
            zFlag = 1;
        end
    end
    ...
    if ~isempty(movingIdx) 
        ...
        if eFlag && zFlag
            tPoints = [tPoints, tPoints(end) + 0.5, tPoints(end) + 1];
        elseif zFlag 
    ...
    %         tPoints = [0, 1, 1.5];
    %     end
    elseif (underToOverFlag)
        tPoints = [0, 1, 1.5]; 
        if eFlag
            tPoints = [0, 1, 1.5, 2];
        end
    elseif (toUnderFlag || overToOverFlag)
        tPoints = [0, 1]; % intermediate sign lasts for 1s
        if eFlag
            tPoints = [0, 1, 1.5];
        end
